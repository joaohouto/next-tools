...
export function RemoveBgView({
  ...
  const [model, setModel] = useLocalStorage<BgModel>(
    "remove-bg-model",
    "isnet_quint8",
  );
    ...
    try {
      ...
      const blob = await removeBackground(file, {
        ...
        progress: (key, current, total) => {
          ...
          let c = 0,
            t = 0;
          for (const [cur, tot] of seen.values()) {
            c += cur;
            t += tot;
          }
          if (t > 0) {
  ...
  const download = () => {
    ...
    Object.assign(document.createElement("a"), {
      href: result,
      download: `${file.name.replace(/\.[^.]+$/, "")}-sem-fundo.png`,
    }).click();
  };
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2">
              {BG_MODELS.map((m) => (
                <button
                  key={m.value}
                  onClick={() => !processing && setModel(m.value)}
                  disabled={processing}
                  className={cn(
            ...
            <p className="text-[11px] text-muted-foreground/60">
              {BG_MODELS.find((m) => m.value === model)?.hint}
            </p>
          </div>
            ...
            <div className="flex gap-2 flex-wrap">
              {result ? (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => doRemoveBg(model)}
                >
                  <RefreshCw size={14} /> Refazer
                </Button>
              ) : (
                <Button size="sm" onClick={() => doRemoveBg(model)}>
                  <Sparkles size={14} /> Remover Fundo
                </Button>
              )}
              ...
              <Button size="sm" onClick={download} disabled={!result}>
